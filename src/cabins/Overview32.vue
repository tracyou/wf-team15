    ...
    <div id="cabinDetails" class="container horizontal-scrollable"><h3>All Cabins Detail (managed by component):</h3>
      <table id="cabinCards">
        <div class="card-block card-1" :class="{'selectedCabinColor': selectCabin === cabin}" v-for="cabin in cabins"
             :key="cabin" @click="getCabin(cabin) ">
      ...
      <div v-else>
        <CabinsDetail32 :selectedCabin="selectCabin" @delete-cabin="deleteCabin"/>
      </div>
    </div>
  </div>
</template>
...
import CabinsDetail32 from "@/cabins/Detail32";

export default {
  name: "Overview32Component",
  components: {CabinsDetail32},
  created() {
    {
      this.lastId = 10000;
      for (let i = 0; i < 8; i++) {
  ...
  methods: {
    ...
    onNewCabin() {
      ...
      this.selectCabin = this.newCabin;
    },
