<template>
  <div class="container">
    <div id="cabinDetails" class="container horizontal-scrollable"><h3>All Cabins Detail (managed by component):</h3>
      ...
      <div v-else>
        <router-view :selectedCabin="cabins.find(value => value === selectCabin)" @delete-cabin="deleteCabin"
                     @unselect-cabin="pushRoutedCabin"></router-view>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    '$route'() {
      if (this.findSelectedFromRouteParam(this.$route) === undefined) {
        ...
        console.log(this.findSelectedFromRouteParam(this.$route));
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    onNewCabin() {
      ...
      this.pushRoutedCabin(this.newCabin);
    },

    deleteCabin(cabin) {
      this.cabins.splice(this.cabins.indexOf(cabin), 1);
      this.pushRoutedCabin(cabin);
    },
    ...
    pushRoutedCabin(cabin) {
      if (cabin != null && cabin !== this.selectCabin) {
        ...
        this.$router.push("/cabins/overview34/ " + titleSlug)
      }
